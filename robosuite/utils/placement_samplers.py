...
from robosuite.utils.transform_utils import quat_multiply, euler2quat
...
class UniformRandomSampler(ObjectPositionSampler):
    ...
    def _sample_quat(self):
        ...
        if self.rotation_axis in ["x", "y", "z"]:
            ...
            euler = np.zeros(3)
            for i, val in enumerate(self.rotation):
                if isinstance(val, tuple):
                    euler[i] = np.random.uniform(high=max(val), low=min(val))
                else:
                    euler[i] = val
